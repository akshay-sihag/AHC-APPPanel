  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-[#dfedfb] overflow-hidden">
        <div className="px-4 py-2 border-b border-[#dfedfb]">
          {canDrag ? (
            ...
            <p className="text-xs text-[#7895b3]">Clear search to enable drag reordering.</p>
          )}
        </div>
              ...
              {filteredBlogs.length === 0 ? (
                ...
                filteredBlogs.map((blog) => (
                  <tr
                    key={blog.id}
                    draggable={canDrag}
                    onDragStart={() => handleDragStart(blog.id)}
                    onDragOver={(e) => handleDragOver(e, blog.id)}
                    onDragLeave={handleDragLeave}
                    onDrop={(e) => handleDrop(e, blog.id)}
                    onDragEnd={() => { setDragId(null); setDragOverId(null); }}
                    className={`transition-colors ${dragOverId === blog.id ? 'bg-[#dfedfb] border-t-2 border-[#435970]' : 'hover:bg-[#dfedfb]/20'} ${dragId === blog.id ? 'opacity-40' : ''}`}
                  >
